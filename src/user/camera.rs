...
pub fn render(player: Player, game_map: &GameMap) {
    clear_background(color::LIME);

    // go through each column on screen and draw walls in the center.
    for (x, wall_height) in get_view(player, game_map).iter().enumerate() {
...
fn horizontal_intersection(player: Player, game_map: &GameMap, angle: f32) -> f32 {
...
fn vertical_intersection(player: Player, game_map: &GameMap, angle: f32) -> f32 {
    let player_x = player.get_x();
    let player_y = player.get_y();
...
fn get_view(player: Player, game_map: &GameMap) -> [i32; RES_HORIZ] {
    ...
    let starting_angle = player.get_theta() + HALF_FOV;
    ...
    for (idx, wall) in walls.iter_mut().enumerate() {
        ...
        let h_dist = horizontal_intersection(player, game_map, angle);
        let v_dist = vertical_intersection(player, game_map, angle);
